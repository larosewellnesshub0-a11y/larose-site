...
export function loadContent() {
  ...
  const neutralArabicReaderAddress = (value) => {
    if (typeof value === "string") {
      const replacements = [
        ...
        ["احجزي", "احجز"], ["تحجزين", "تحجز"], ["اكتبي", "اكتب"], ["ابدئي", "ابدأ"], ["استخدمي", "استخدم"],
        ["اجعلي", "اجعل"], ["تحاولي", "تحاول"], ["تتلقينها", "تتلقاها"], ["تغيري", "تغير"],
        ["اتبعي", "اتبع"], ["اسألي", "اسأل"], ["اختاري", "اختار"],
        ["راجعي", "راجع"], ["تستخدمي", "تستخدم"], ["تحجزي", "احجز"],
        ["حضّري", "حضّر"], ["تحتاجين", "تحتاج"], ["اذكريه", "اذكره"], ["اذكري", "اذكر"],
  ...
  const colonoscopyPreparationRewrite = read("articles-colonoscopy-preparation-rewrite-2026-09-17.json");
  const normalUltrasoundRewrite = read("articles-normal-ultrasound-rewrite-2026-09-17.json");
  ...
  const foodIntoleranceAllergyRewrite = read("articles-food-intolerance-allergy-rewrite-2026-09-18.json");
  const lowFerritinNormalCbcRewrite = read("articles-low-ferritin-normal-cbc-rewrite-2026-09-18.json");
  const gallstonesSymptomsSurgeryRewrite = read("articles-gallstones-symptoms-surgery-rewrite-2026-09-18.json");
  const silentGallstonesRewrite = read("articles-silent-gallstones-rewrite-2026-09-20.json");
  const hPyloriChildrenSymptomsRewrite = read("articles-h-pylori-children-symptoms-rewrite-2026-09-20.json");
  const responsiveComplementaryFeedingRewrite = read("articles-responsive-complementary-feeding-rewrite-2026-09-20.json");
  ...
  const preparingForBariatricSurgeryRewrite = read("articles-preparing-for-bariatric-surgery-rewrite-2026-09-20.json");
  const insulinResistanceChildrenRewrite = read("articles-insulin-resistance-children-rewrite-2026-09-21.json");
  const childrensAppetiteRewrite = read("articles-childrens-appetite-rewrite-2026-09-21.json");
  const protectingMuscleRewrite = read("articles-protecting-muscle-while-losing-weight-rewrite-2026-09-21.json");
  const vitaminB12PlantBasedRewrite = read("articles-vitamin-b12-plant-based-rewrite-2026-09-21.json");
  ...
  const bodyContouringSafetyRewrite = read("articles-body-contouring-safety-rewrite-2026-09-21.json");
  const ironDeficiencyChildrenRewrite = read("articles-iron-deficiency-children-rewrite-2026-09-21.json");
  const hairLossChildrenRewrite = read("articles-hair-loss-children-rewrite-2026-09-21.json");
  ...
  const celluliteRealisticOptionsRewrite = read("articles-cellulite-realistic-options-and-limits-rewrite-2026-09-21.json");
  const bodyContouringVsWeightLossRewrite = read("articles-body-contouring-vs-weight-loss-rewrite-2026-09-21.json");
  const highCholesterolRewrite = read("articles-high-cholesterol-results-and-next-steps-rewrite-2026-09-21.json");
  const fattyLiverSymptomsDiagnosisRewrite = read("articles-fatty-liver-symptoms-diagnosis-risk-rewrite-2026-09-21.json");
  ...
  articles.articles = [
    ...
    ...(colonoscopyPreparationRewrite.articles || []),
    ...(normalUltrasoundRewrite.articles || []),
    ...
    ...(foodIntoleranceAllergyRewrite.articles || []),
    ...(lowFerritinNormalCbcRewrite.articles || []),
    ...(gallstonesSymptomsSurgeryRewrite.articles || []),
    ...(silentGallstonesRewrite.articles || []),
    ...(hPyloriChildrenSymptomsRewrite.articles || []),
    ...(responsiveComplementaryFeedingRewrite.articles || []),
    ...
    ...(preparingForBariatricSurgeryRewrite.articles || []),
    ...(insulinResistanceChildrenRewrite.articles || []),
    ...(childrensAppetiteRewrite.articles || []),
    ...(protectingMuscleRewrite.articles || []),
    ...(vitaminB12PlantBasedRewrite.articles || []),
    ...
    ...(bodyContouringSafetyRewrite.articles || []),
    ...(ironDeficiencyChildrenRewrite.articles || []),
    ...(hairLossChildrenRewrite.articles || []),
    ...
    ...(celluliteRealisticOptionsRewrite.articles || []),
    ...(bodyContouringVsWeightLossRewrite.articles || []),
    ...(highCholesterolRewrite.articles || []),
    ...(fattyLiverSymptomsDiagnosisRewrite.articles || []),
