...
- Deployment: **not yet pushed in this session** — awaiting the user's go-ahead on this batch of source changes before commit/push, per "confirm before push" defaults. `site/` was left unstaged/uncommitted; GitHub Actions (`.github/workflows/deploy.yml`) rebuilds `site/` from source on every push to `main`, so the local generated output does not need to be committed.
